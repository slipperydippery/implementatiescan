...
var _SubActie = require('../components/SubActie.vue');

var _SubActie2 = _interopRequireDefault(_SubActie);
...
exports.default = {
	...
	methods: {
		...
		newSubactie: function newSubactie() {
			var home = this;
		...
		toggleWerkActieActive: function toggleWerkActieActive(actie) {
			actie.werkactive = !actie.werkactive;
			this.saveActie();
		},

		getSubActies: function getSubActies() {
			var _this2 = this;

			this.$http.get('/api/verbeteractie/' + this.actie.id + '/subactie').then(function (response) {
...
exports.default = {
	...
	methods: {
		...
		getInstanties: function getInstanties() {
			...
			var home = this;
			this.$http.get('/api/scan/' + this.scan.id + '/instantie').then(function (response) {
				_this2.instanties = response.data;
			});
		},
	...
	computed: {
		...
		searchedLinks: function searchedLinks() {
			...
			return this.links.filter(function (link) {
				if (link.title.toLowerCase().indexOf(self.search.toLowerCase()) !== -1 || link.adress.toLowerCase().indexOf(self.search.toLowerCase()) !== -1) {
					return true;
				}
			});
		}
...
if (module.hot) {(function () {  module.hot.accept()
  ...
  hotAPI.install(require("vue"), true)
  if (!hotAPI.compatible) return
  var id = "/Users/silvernitrate/Code/quest/resources/assets/js/components/Links.vue"
  module.hot.dispose(function () {
    require("vueify-insert-css").cache["\n\t.searchfilter {\n\t\ttext-align: right;\n\t\tlabel, input {\n\t\t\tdisplay: inline-block;\n\t\t}\n\t}\n"] = false
    document.head.removeChild(__vueify_style__)
  })
	...
	computed: {
		...
		searchedAndFilteredPdfs: function searchedAndFilteredPdfs() {
			...
			return this.pdfs.filter(function (pdf) {
				if (pdf.organisation.toLowerCase().indexOf(self.search.toLowerCase()) !== -1 || pdf.description.toLowerCase().indexOf(self.search.toLowerCase()) !== -1 || pdf.publication_year.toLowerCase().indexOf(self.search.toLowerCase()) !== -1 || pdf.adress.toLowerCase().indexOf(self.search.toLowerCase()) !== -1) {
					return true;
				}
			});
		}
...
exports.default = {
	...
	methods: {
		...
		getWhoIncomplete: function getWhoIncomplete() {
			var _this3 = this;

			this.$http.get('/api/scan/' + this.scan.id + '/thema/' + this.thema_id + '/getWhoIncomplete').then(function (response) {
				_this3.whoUnanswered = response.data;
			});
		},
...
exports.default = {
	...
	props: ['participant', 'editable', 'instanties'],
...
exports.default = {
	...
	methods: {
		...
		getBetrokkenen: function getBetrokkenen() {
			...
			this.$http.get('/api/subactie/' + this.subactie.id + '/betrokkene').then(function (response) {
				_this.betrokkenen = response.data;
			});
			this.$http.get('/api/subactie/' + this.subactie.id + '/unbetrokkene').then(function (response) {
				_this.unBetrokkenen = response.data;
			});
		},
		...
		getExternalusers: function getExternalusers() {
			...
			this.$http.get('/api/subactie/' + this.subactie.id + '/externaluser').then(function (response) {
				_this2.externalUsers = response.data;
			});
		},
...
var _Scanlist = require('./components/Scanlist.vue');
...
var _DeelnemersMenuItem = require('./components/DeelnemersMenuItem.vue');

var _DeelnemersMenuItem2 = _interopRequireDefault(_DeelnemersMenuItem);
...
new Vue({
	...
	components: {
		...
		DeelnemersMenuItem: _DeelnemersMenuItem2.default
	},
